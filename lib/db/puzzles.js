...
Proto.removePuzzle = function(puzzleId, cb) {
  var self = this;
  async.parallel([
    function(cb) {
      self.redis.zrem([PUZZLES_KEY, puzzleId], cb);
    },
    function(cb) {
      self.redis.del(piecesKey(puzzleId), cb);
    },
    function(cb) {
      self.redis.del(puzzleKey(puzzleId), cb);
    }
  ], cb);
};
...
Proto.setPieces = function(puzzleId, pieces, cb) {
  ...
  this.redis.del(key, function(err) {
    if (err) return cb(err);
    self.redis.rpush(args, function(err) {
      if (err) return cb(err);
      cb();
    });
  });
...
Proto.getPuzzleIdByIndex = function(index, cb) {
  var args = [PUZZLES_KEY, index, index];
...
Proto.getPuzzlesIDs = function(op, cb) {
...
Proto.getPuzzleStatus = function(puzzleId, cb) {
  this.getPieces(puzzleId, function(err, pieces) {
    ...
    cb(null, Math.ceil(count / (pieces.length / 100)));
  });
};

Proto.incrPuzzleComplete = function(puzzleId, cb) {
  var args = [puzzleKey(puzzleId), 'complete', 1];
  this.redis.hincrby(args, function(err) {
    if (err) return cb(err);
    cb();
  });
};
